...
const EXAMPLES_SOURCES = Object.keys(Helpers)
	.map((desc) => ({ desc, value: Helpers?.[desc] }))
	.map(({ desc, value }) => ({ desc, value, type: typeof value }))
	.map(({ desc, value, type }, index) => ({
		desc,
		value,
		index,
		isFunc: type === "function",
		text: `${index}. Name: ${desc}, Type: ${typeof value}`,
		example: type === "function" ? () => value : value
	}))

const EXAMPLES_CONSTANTS = EXAMPLES_SOURCES.filter(({ isFunc }) => isFunc !== true)
const EXAMPLES_FUNCTIONS = EXAMPLES_SOURCES.filter((v) => v.isFunc).map((v) => {
	const { value, desc } = v
	const callback = (...v) => {
		console.log(`Run ${desc} with args:`, ...v)
		try {
	...
	return { ...v, desc, value, callback }
})
const EXAMPLES_FUNCTION_NAMES = EXAMPLES_FUNCTIONS.reduce((acc, v) => [...acc, v.desc], [])
const EXAMPLES_FUNCTION_RESULTS = [
	{ desc: "FUNC", result: Helpers.FUNC() },
	{ desc: "_logGradient", result: Helpers._logGradient() },
	{ desc: "_logColored", result: Helpers._logColored() },
	{ desc: "_logLolcatPrint", result: Helpers._logLolcatPrint() },
	{ desc: "_logLolcatRainbow", result: Helpers._logLolcatRainbow() },
	{ desc: "_logCowSay", result: Helpers._logCowSay() },
	{ desc: "_logCowThink", result: Helpers._logCowThink() },
	{ desc: "_logDefault", result: Helpers._logDefault() },
	{ desc: "_logJson", result: Helpers._logJson() },
	{ desc: "_logValues", result: Helpers._logValues() },
	{ desc: "_logExamples", result: Helpers._logExamples() },
...
const EXAMPLES_JSON = Helpers.toText({
	EXAMPLES_SOURCES,
	EXAMPLES_CONSTANTS,
	EXAMPLES_FUNCTIONS,
...
Helpers.fileWrite("helpersExamples.json", EXAMPLES_JSON)
console.log("Saved!")
