...
const addProps = (v, i) => ({
	...v,
	isTrue: v.result > v.output[0] - 0.1 && v.result < v.output[0] + 0.1,
...
const RESULTS_SUM = SUM.map((v) => ({ ...v, result: NET_SUM.activate(v.input) })).map(addProps)
const RESULTS_AVERAGE = AVERAGE.map((v) => ({ ...v, result: NET_AVERAGE.activate(v.input) })).map(addProps)
const RESULTS_DOUBLE = DOUBLE.map((v) => ({ ...v, result: NET_DOUBLE.activate(v.input) })).map(addProps)
...
Values.writeOutput(JSON.stringify({ NET_DOUBLE: NET_DOUBLE.toJSON() }, null, 2), "./results/NET_DOUBLE.txt")
